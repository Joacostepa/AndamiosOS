...
export async function enviarMail(m: { para: string; cc?: string[]; asunto: string; texto: string }): Promise<void> {
  ...
  await transporte.sendMail({ from: user, to: m.para, cc: m.cc, subject: m.asunto, text: m.texto });
}
